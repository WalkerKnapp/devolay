...
public class SendAudioExample {
    public static void main(String[] args) {
        ...
        final int sampleCount = sampleRate / 10;

        final ByteBuffer data = ByteBuffer.allocateDirect((sampleCount * channelCount * Float.SIZE) / Byte.SIZE);
        ...
        for(int i = 0; i < totalSamplesToSend; i += sampleCount) {
            ...
            for(int ch = 0; ch < channelCount; ch++) {
                for(int sample = 0; sample < sampleCount; sample++) {
                    ...
                    floatData.put((float) Math.sin(sample * 2f * Math.PI * frequencyPerChannel[ch]));
